...
def resolve_shared_dir(env_var: str, default_root: str) -> Path:
    ...
    return Path(f"/scratch/{user}/{default_root}/stage4")
...
def _load_dataset(cfg: dict):
    if cfg.get("dataset_path"):
        dataset_path = os.path.expanduser(os.path.expandvars(str(cfg["dataset_path"])))
...
def _build_cf_example(task: str, example: dict) -> Optional[dict]:
    ...
    if task_lower == "hellaswag":
    ...
    elif task_lower in {"arc_easy", "arc-easy", "arc", "arc_challenge", "arc-challenge"}:
        stem = None
        choices = None
    ...
    elif task_lower in {"commonsense_qa", "commonsenseqa", "cqa"}:
        stem = example.get("question")
    ...
    elif task_lower in {"social_i_qa", "socialiqa"}:
        context = example.get("context")
        question = example.get("question")
        ans1 = example.get("answerA")
        ans2 = example.get("answerB")
        ans3 = example.get("answerC")
        label = example.get("label")
        if context is None or question is None or ans1 is None or ans2 is None or ans3 is None or label is None:
            return None
        prompt = f"{context}\nQuestion: {question}\nAnswer:"
        ...
        if str(label).isdigit():
            ...
            label_idx = {"A": 0, "B": 1, "C": 2}.get(str(label).strip().upper(), -1)
        if label_idx < 0:
            return None

    elif task_lower == "sciq":
        stem = example.get("question")
        correct = example.get("correct_answer")
        d1 = example.get("distractor1")
        d2 = example.get("distractor2")
        d3 = example.get("distractor3")
        if stem is None or correct is None or d1 is None or d2 is None or d3 is None:
            return None
        prompt = stem
        candidates = [correct, d1, d2, d3]
        label_idx = 0

    elif task_lower == "qasc":
        stem = example.get("question")
        if isinstance(stem, dict):
            stem = stem.get("stem")
        choices = example.get("choices")
    ...
    elif task_lower == "copa":
        premise = example.get("premise")
        choice1 = example.get("choice1")
        choice2 = example.get("choice2")
        question = example.get("question")
        label = example.get("label")
        if premise is None or choice1 is None or choice2 is None or question is None or label is None:
            return None
        prompt = f"Premise: {premise}\nQuestion: {question}\nAnswer:"
        candidates = [choice1, choice2]
        label_idx = int(label)

    elif task_lower == "mmlu":
        question = example.get("question")
        choices = example.get("choices")
        answer = example.get("answer")
        ...
        candidates = [str(c) for c in list(choices)]
        labels = [chr(ord("A") + i) for i in range(len(candidates))]
        prompt = _format_mc_prompt(str(question), labels, candidates)

        if isinstance(answer, int):
            label_idx = answer
        elif str(answer).isdigit():
            label_idx = int(answer)
        else:
            label_idx = _label_to_index(str(answer), labels)

        if label_idx < 0 or label_idx >= len(candidates):
            return None
...
def prepare_sft_native_prompts(cfg: dict, eval_data_dir: Path) -> Tuple[List[dict], dict]:
    sft_dir = eval_data_dir / "sft_native"
    ensure_dir(sft_dir)
    prompts_path = sft_dir / "prompts.jsonl"
    meta_path = sft_dir / "meta.json"

    configured_prompts = cfg.get("prompts", [])
    if not configured_prompts:
        raise RuntimeError("Stage-4 Tier4 prompt suite is empty")

    version = int(cfg.get("version", 1))
    expected_count = len(configured_prompts)

    if prompts_path.exists() and meta_path.exists():
        meta = json.loads(meta_path.read_text(encoding="utf-8"))
        prompts = _read_jsonl(prompts_path)
        if meta.get("version") == version and len(prompts) == expected_count:
            return prompts, meta

    prompts: List[dict] = []
    for idx, prompt_cfg in enumerate(configured_prompts, start=1):
        user_text = str(prompt_cfg.get("user", "")).strip()
        if not user_text:
            raise RuntimeError(f"Tier4 prompt at index {idx} has empty 'user'")
        prompt = {
            "id": str(prompt_cfg.get("id", f"prompt_{idx:03d}")),
            "category": str(prompt_cfg.get("category", "general")),
            "system": str(prompt_cfg.get("system", "You are a helpful assistant.")),
            "user": user_text,
    ...
    prompts_path.write_text("\n".join(json.dumps(prompt, ensure_ascii=False) for prompt in prompts), encoding="utf-8")
    meta_path.write_text(json.dumps(meta, indent=2), encoding="utf-8")
    return prompts, meta
...
def run_lighteval(
    ...
    if not cmd:
        raise RuntimeError("LIGHTEVAL_CMD is required for Nanotron Stage-4 evaluation")
    env = os.environ.copy()
    env.update(
        {
    ...
    tier4_path = results_dir / "tier4_sft_native.json"
    if not tier2_path.exists() or not tier3_path.exists() or not tier4_path.exists():
        raise RuntimeError("LightEval did not produce tier2/tier3/tier4 outputs")
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    ppl_cfg_path = Path(__file__).resolve().parent / "task_suites" / "stage4_ppl_slices.yaml"
    cf_cfg_path = Path(__file__).resolve().parent / "task_suites" / "stage4_cf_core.yaml"
    sft_cfg_path = Path(__file__).resolve().parent / "task_suites" / "stage4_sft_native.yaml"
    ...
    _ = prepare_ppl_slices(ppl_cfg, tokenizer, args.seq_len, eval_data_dir)
    log("Preparing CF tasks")
    _ = prepare_cf_tasks(cf_cfg, eval_data_dir)
    log("Preparing SFT-native prompts")
    _ = prepare_sft_native_prompts(sft_cfg, eval_data_dir)

    tier2, tier3, tier4 = run_lighteval(
    ...
    write_json(results_dir / "tier4_sft_native.json", tier4)

...
def resolve_shared_dir(env_var: str, default_root: str) -> Path:
    ...
    return Path(f"/scratch/{user}/{default_root}/stage2")
...
def run_lighteval(
    ...
    if not cmd:
        raise RuntimeError("LIGHTEVAL_CMD is required for Nanotron Stage-2 evaluation")
    env = os.environ.copy()
    env.update(
        {
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    ppl_cfg_path = Path(__file__).resolve().parent / "task_suites" / "stage2_ppl_slices.yaml"
    cf_cfg_path = Path(__file__).resolve().parent / "task_suites" / "stage2_cf_core.yaml"

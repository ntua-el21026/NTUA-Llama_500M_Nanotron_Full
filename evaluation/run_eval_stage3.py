...
def resolve_shared_dir(env_var: str, default_root: str) -> Path:
    ...
    return Path(f"/scratch/{user}/{default_root}/stage3")
...
def run_lighteval(
    ...
    if not cmd:
        raise RuntimeError("LIGHTEVAL_CMD is required for Nanotron Stage-3 evaluation")
    env = os.environ.copy()
    env.update(
        {
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    ppl_cfg_path = Path(__file__).resolve().parent / "task_suites" / "stage3_ppl_slices.yaml"
    cf_cfg_path = Path(__file__).resolve().parent / "task_suites" / "stage3_cf_core.yaml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 3 evaluation runner")

#!/usr/bin/env python3
# Project path: docs/project_analytics/project_analytics.py
"""
project_analytics.py

Locate the project root from this repository, then:
...
4. Write a single report to:
   <script_directory>/project_analytics.txt
...
def classify_file(path: Path) -> str:
    """
    Return a visible category for every file.
    - Known extensions are mapped via LANGUAGE_MAP.
    - Unknown extensions are exposed as 'Ext/<ext>'.
    - Extensionless files use FILE_NAME_MAP when possible, else 'No Extension'.
    """
    ext = path.suffix.lower()
    if ext in LANGUAGE_MAP:
        return LANGUAGE_MAP[ext]
    if ext:
        return f"Ext/{ext.lstrip('.')}"
    return FILE_NAME_MAP.get(path.name.lower(), "No Extension")


def find_project_root(start: Path) -> Path:
    """
    Prefer the closest ancestor containing .git. If missing, fall back to
    ...
    while True:
        ...
        if any((current / marker).exists() for marker in (
            "pyproject.toml", "requirements.txt", "setup.py", "README.md",
            "config", "scripts", "evaluation"
        )):
            logger.debug(f"Found project root (marker fallback) at {current}")
            return current
...
def load_ignore_patterns(root: Path) -> PathSpec:
    """
    Read .gitignore under `root` using standard semantics:
...
def analyze_tree(
    ...
    for idx, path in enumerate(non_ignored):
        ...
        if path.is_dir():
            ...
            # Treat assets/binaries as 0 logical code lines.
            if language.startswith("Asset/") or language in NON_TEXT_CATEGORIES:
                count_lines = 0
            else:
                try:
...
def main() -> None:
    ...
    dirs_wi, files_wi, lines_wi, stats_wi = analyze_tree(project_root, real_ignore_spec, quiet=quiet)
    logger.info(
        f"[With .gitignore]  {dirs_wi:,} dirs, {files_wi:,} files, {lines_wi:,} lines"
    )

    output_file = script_dir / "project_analytics.txt"
    try:

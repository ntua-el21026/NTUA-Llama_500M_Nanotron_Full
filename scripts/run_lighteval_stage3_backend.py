...
def _load_eval_module(repo_root: Path):
    eval_path = repo_root / "evaluation" / "run_eval_stage3.py"
    spec = importlib.util.spec_from_file_location("stage3_eval_impl", str(eval_path))
...
def _validate_checkpoint_layout(checkpoint_path: Path) -> None:
    ...
    if tp != 1:
        raise RuntimeError(
            ...
            "Stage-3 Nanotron scorer currently supports tp=1 checkpoints only."
        )
    if cp != 1:
        raise RuntimeError(
            f"Unsupported checkpoint context parallel size cp={cp}. "
            "Stage-3 Nanotron scorer currently supports cp=1 checkpoints only."
        )


def main() -> None:
    repo_root = Path(_require_env("STAGE3_REPO_ROOT"))
    eval_data_dir = Path(_require_env("STAGE3_EVAL_DATA_DIR"))
    results_dir = Path(_require_env("STAGE3_RESULTS_DIR"))
    ...
    print(f"[stage3-real] wrote {results_dir / 'tier2_ppl.json'}", flush=True)
    print(f"[stage3-real] wrote {results_dir / 'tier3_cf.json'}", flush=True)
    if nan_tracker["nan_inf_count"] > 0:
        print(f"[stage3-real] warning: nan/inf batches={nan_tracker['nan_inf_count']}", flush=True)

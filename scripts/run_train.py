...
# # 1. Κρατάμε το αυθεντικό Init
# original_init = DatatroveFolderDataset.__init__

# def patched_init(self, *args, **kwargs):
#     # 2. Βρίσκουμε τι δέχεται ΠΡΑΓΜΑΤΙΚΑ η έκδοση του Datatrove που έχουμε
#     sig = inspect.signature(original_init)
#     valid_params = sig.parameters.keys()

#     # 3. Μετονομάζουμε τα βασικά
#     if "folder_path" in kwargs:
#         kwargs["data_folder"] = kwargs.pop("folder_path")
#     if "eos_token_id" in kwargs and "positions_from_eos_token_id" in valid_params:
#         kwargs["positions_from_eos_token_id"] = kwargs.pop("eos_token_id")

#     # 4. ΠΕΤΑΜΕ οτιδήποτε άλλο δεν αναγνωρίζει το Datatrove (max_tokens, skip_in_stream, κλπ)
#     # Αυτό καθαρίζει αυτόματα όλα τα TypeError
#     final_kwargs = {k: v for k, v in kwargs.items() if k in valid_params}

#     return original_init(self, *args, **final_kwargs)

# # 5. Εφαρμογή του Patch
# DatatroveFolderDataset.__init__ = patched_init
...
if __name__ == "__main__":
    ...
    # ΠΡΟΣΘΗΚΗ ΓΙΑ ΔΙΑΓΝΩΣΗ
    if dist.get_rank() == 0:
        print("--- GPU MEMORY PROFILING START ---")
        print(torch.cuda.memory_summary(device=None, abbreviated=False))
        print("--- GPU MEMORY PROFILING END ---")
    # Train

...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("stage1_eval_impl", str(path))
...
repo_root = Path.cwd()
eval_impl = load_module(repo_root / "evaluation" / "run_eval_stage1.py")

checkpoint_stub = Path("/tmp/stage1_cache_prefill")
tokenizer = eval_impl.load_tokenizer(tokenizer_name, checkpoint_stub)
ppl_cfg = eval_impl.load_yaml(repo_root / "evaluation" / "task_suites" / "stage1_ppl_slices.yaml")
cf_cfg = eval_impl.load_yaml(repo_root / "evaluation" / "task_suites" / "stage1_cf_core.yaml")

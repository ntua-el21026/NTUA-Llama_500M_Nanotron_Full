...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("stage4_eval_impl", str(path))
...
repo_root = Path.cwd()
eval_impl = load_module(repo_root / "evaluation" / "run_eval_stage4.py")

checkpoint_stub = Path("/tmp/stage4_cache_prefill")
tokenizer = eval_impl.load_tokenizer(tokenizer_name, checkpoint_stub)
ppl_cfg = eval_impl.load_yaml(repo_root / "evaluation" / "task_suites" / "stage4_ppl_slices.yaml")
cf_cfg = eval_impl.load_yaml(repo_root / "evaluation" / "task_suites" / "stage4_cf_core.yaml")
sft_cfg = eval_impl.load_yaml(repo_root / "evaluation" / "task_suites" / "stage4_sft_native.yaml")
...
eval_impl.log("Prefill: preparing Tier4 SFT-native prompts")
eval_impl.prepare_sft_native_prompts(sft_cfg, eval_data_dir)
...
echo "Tier4 prompt file:   $(find "$EVAL_DATA_DIR/sft_native" -name prompts.jsonl 2>/dev/null | wc -l)"
